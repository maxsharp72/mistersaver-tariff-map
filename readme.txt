...
* Opacity повышена до 0.85 (вместо 0.7) — значок лучше виден

= 0.2.22 =
* reCAPTCHA-бейдж теперь перенесён в левый нижний угол (вместо скрытия) — больше не перекрывает кнопку «вверх», остаётся видимым согласно TOS Google
* Прозрачность бейджа 0.7 (1.0 при наведении), плавный transition
* На мобильных бейдж уменьшен до 85%
* Убрано автозаполнение поля «Сообщение» на /contacts/ — больше не показывает HTML-entities вроде &#1057;&#1090;&#1088;...
* Поле «Тема» по-прежнему автозаполняется (контекста для пользователя достаточно)

= 0.2.21 =
* Скрыт reCAPTCHA-бейдж (.grecaptcha-badge) — больше не закрывает кнопку «вверх»
* Добавлен блок «Нашли ошибку?» в конце страниц регионов и архива с переходом на /contacts/?subject=...&ref=...
* JS-автозаполнение полей формы контактов (Тема + Сообщение) из URL-параметров
* Анимированные ховеры на партнёрских кнопках: подъём -2px, свечение тени, масштабирование иконки
...
= 0.2.18 =
* Автоматический трекинг кликов по партнёркам в Яндекс.Метрике: поле «ID счётчика» в настройках плагина → JS-снипет автоматически выводится на страницах региона/архива. Не нужны WPCode или Astra Pro Custom Code.
